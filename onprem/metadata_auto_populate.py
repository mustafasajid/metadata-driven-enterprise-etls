...
parser = argparse.ArgumentParser(description="Auto-populate metadata tables (on-premises)")
...
spark = SparkSession.builder.appName("MetadataAutoPopulate").getOrCreate()
metadata = MetadataManager(spark)
conn_meta = metadata.load_connection_metadata()

conn = conn_meta.filter(conn_meta.connection_id == args.connection_id).collect()[0]

jdbc_url = None
table_query = None
if conn.type == "oracle":
    jdbc_url = f"jdbc:oracle:thin:@{conn.host}:{conn.port}/{conn.database}"
    table_query = f"SELECT table_name FROM all_tables WHERE owner = '{args.schema_name.upper()}'"
elif conn.type == "sqlserver":
    jdbc_url = f"jdbc:sqlserver://{conn.host}:{conn.port};databaseName={conn.database}"
    table_query = f"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{args.schema_name}'"
    ...
    jdbc_url = f"jdbc:snowflake://{conn.host}/?db={conn.database}&schema={args.schema_name}"
    table_query = f"SELECT table_name FROM information_schema.tables WHERE table_schema = '{args.schema_name.upper()}'"
elif conn.type == "bigquery":
    jdbc_url = f"jdbc:bigquery://https://www.googleapis.com/bigquery/v2:443;ProjectId={conn.database};"
    table_query = f"SELECT table_name FROM `{args.schema_name}.INFORMATION_SCHEMA.TABLES`"
elif conn.type == "redshift":
    jdbc_url = f"jdbc:redshift://{conn.host}:{conn.port}/{conn.database}"
    table_query = f"SELECT tablename AS table_name FROM pg_table_def WHERE schemaname = '{args.schema_name}'"
elif conn.type == "db2":
    jdbc_url = f"jdbc:db2://{conn.host}:{conn.port}/{conn.database}"
    table_query = f"SELECT tabname AS table_name FROM syscat.tables WHERE tabschema = '{args.schema_name.upper()}'"
elif conn.type == "hana":
    jdbc_url = f"jdbc:sap://{conn.host}:{conn.port}"
    table_query = f"SELECT TABLE_NAME FROM TABLES WHERE SCHEMA_NAME = '{args.schema_name.upper()}'"
else:
...
for tbl in table_names:
    table_id = f"{args.connection_id}_{tbl}"
    tgt_tbl_name = f"{args.target_prefix}{tbl}{args.target_suffix}" if args.target_prefix or args.target_suffix else tbl
    ...
    col_query = get_column_query(conn.type, args.schema_name, tbl)
    col_options = {
        "url": jdbc_url,
        "user": conn.username,
        "password": conn.password,
...
print(f"Populated table_metadata for {len(table_names)} tables and column_metadata for {len(column_metadata_rows)} columns from schema {args.schema_name} (on-premises version).")